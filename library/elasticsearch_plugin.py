...
def install_plugin(module, plugin_bin, plugin_name, version, url, proxy_host, proxy_port, timeout):
    ...
    es_version = get_elasticsearch_version(module, plugin_bin)
    if es_version <= 5.3:
        if timeout:
...
def get_elasticsearch_version(module, plugin_bin):
    es_bindir = os.path.dirname(plugin_bin)

    cmd = es_bindir + '/elasticsearch' + ' --version'

    rc, out, err = module.run_command(cmd)

    es_version = out.split(',')[0][9:12]
    # es_version = float(es_version)
